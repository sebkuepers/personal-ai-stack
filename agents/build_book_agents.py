"""Generate the agent definitions of the book domain (``agents/book-*.json``).

Why a generator and not hand-written JSON: the ``response_format`` schema has to
match the Pydantic models in ``workflows/book/models.py`` exactly. Maintained by
hand the two are guaranteed to drift — and the deviation then only surfaces when
an agent delivers something in production that nobody can validate.

How it works:

* **Schema** is always regenerated from the Pydantic model.
* **Instructions** live here as readable constants and are written too — they
  are the substance and belong under version control. They are German because
  the agents work on German prose.
* An existing ``id`` field stays **untouched**, so that an agent already created
  in Studio is updated and not created twice.

Afterwards run ``agents/sync.py``, which plays the files into Studio.

    uv run --project workflows python agents/build_book_agents.py
...
from workflows.book import config as c  # noqa: E402
...
7. Bei Unsicherheit: confidence unter 0.8 setzen und in "why" sagen, warum du zögerst. Lieber
...
1. Jeder Vorschlag nennt in "rule_id" die Regel, gegen die die Stelle VERSTOESST — nicht die
   Regel, die sich hier noch anwenden liesse. Findest du keine, schreibe "kein-bezug"; solche
   Vorschlaege werden nur bei severity = "hoch" angezeigt. Erfinde keine Regel-ID.
...
9. "why" nennt in EINEM Satz, wonach die Stelle stattdessen klingt. Kein Schreibratgeber-Ton.
...
1. "chapter_thesis": Wovon handelt dieses Kapitel, in EINEM Satz — gelesen aus dem Text, nicht aus
   der Rubrik. Der Vergleich der beiden ist oft der wichtigste Befund überhaupt.

2. "touchstones" IST PFLICHT und wird ZUERST ausgefüllt. Zu JEDER mitgelieferten Frage ein
   Urteil — "holds", "wobbles" oder "fails" — mit Begründung und den Abschnittstiteln, an denen
...
3. "carries" IST PFLICHT. Zu JEDEM Punkt aus "muss tragen" — ausnahmslos jedem, auch den
   offensichtlichen — ein Urteil: yes, partly, no. Bei "yes" oder "partly" nennst du die
   Abschnitte, in denen es steht. Bei "no" sagst du, was fehlt. Die Liste hat genau so viele
...
4. "surplus": Was laut Rubrik NICHT getragen werden muss, aber trotzdem im Kapitel steht.

5. "cut_candidates": Stellen, die das Kapitel nicht braucht — mit Abschnitt, Umfang und Grund.
   Sei zurückhaltend. Ein Kapitel, aus dem nichts zu streichen ist, gibt es.

6. "questions_for_the_author": HÖCHSTENS FÜNF. Nur Fragen, die sich von außen nicht entscheiden lassen
...
1. Zu JEDER vorgelegten Regel genau ein Urteil. "rule_id" ist die mitgelieferte Kennung.

2. Sei streng, aber nicht spitzfindig. Eine Fundstelle muss die Regel zeigen, nicht beweisen.
   Zeigt sie sie teilweise oder nur in einem Nebensatz, ist das ein true.

3. "why" in einem halben Satz: Was man im Satz sieht — oder was fehlt.
...
VIER URTEILE:
  ...
  holds                Die Stelle klingt so, wie sie dasteht, nach jemand anderem, und der
                       Vorschlag bringt sie zum Autor zurueck. Nur dann.

  inverts_the_rule     Die zitierte Regel sagt das Gegenteil dessen, was der Vorschlag tut.
                       Beispiel: Das Profil fuehrt Praeteritum FUER Rueckblenden, der Vorschlag
                       zieht eine Rueckblende ins Praesens.

  no_violation         Die Stelle klingt bereits nach dem Autor. Der Vorschlag will die Regel nur
                       staerker anwenden. Erkennbar daran, dass die Begruendung sinngemaess
                       lautet "hier koennte man X auch noch machen".

  overreaches          Der Kern stimmt, die Aenderung geht darueber hinaus: veraendert die
                       Bedeutung, fasst direkte Rede oder Redebegleiter an, streicht konkrete
                       Angaben (Farben, Zahlen, Namen) als angeblich wertend.

REGELN:

1. Zu JEDEM vorgelegten Vorschlag genau ein Urteil. "number" ist seine Position in der Liste, ab 1.
...
3. "why" in einem Satz, konkret. Bei "holds" sagst du, wonach die aktuelle Fassung klingt.

4. "verdict" fasst in einem Satz zusammen, wie die Liste insgesamt dasteht.
...
1. "missed" enthält Fehler, die der erste Lektor nicht gemeldet hat. Jeder mit "paragraph_index"
...
3. "unfounded" verweist mit "number" auf die Position in der vorgelegten Liste, ab 1.
...
5. Sei nicht ehrgeizig. Findest du nichts, sind beide Listen leer, und "verdict" sagt das in einem
...
AGENTS = [
    {
        "file": "book-voice-sample.json",
        "name": "Book · Voice sample",
        "description": (
...
def main() -> int:
    ...
    for a in AGENTS:
        ...
        definition |= {
            ...
            "description": a["description"],
            "tools": [],
            "metadata": {"domain": "book"},
        }

        path.write_text(
            json.dumps(definition, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
        )
        state = "aktualisiert" if existing else "neu"
        print(f"  {a['file']:28} {state:13} ({a['model_cls'].__name__})")
